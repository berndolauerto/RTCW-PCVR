...
*/
#ifdef AUTOAIM
void CL_SetUserCmdValue( int userCmdValue, int holdableValue, float sensitivityScale, int cld, int lastfire, int crosshairstat ) {
	cl.cgameLastFireTime = lastfire;
	cl.cgameCrossHairStatus = crosshairstat;
#else
...
//		return 0;
	...
	case CG_SETUSERCMDVALUE:
		#ifdef AUTOAIM
		CL_SetUserCmdValue( args[1], args[2], VMF( 3 ), args[4], args[5], args[6] );    //----(SA)	modified	// NERVE - SMF - added fourth arg [cld], *SEB* added 5 (lastfire) and 6 (crosshairstat)
		#else
